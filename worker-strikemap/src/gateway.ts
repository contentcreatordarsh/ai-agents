/** Legacy intel-map edge APIs + EC2 proxy (SE assignment paths). */
...
async function proxyToEc2(request: Request, env: GatewayEnv): Promise<Response> {
...
async function getVotes(env: GatewayEnv): Promise<Record<string, number>> {
...
async function validCalloutIds(request: Request, env: GatewayEnv): Promise<Set<string>> {
